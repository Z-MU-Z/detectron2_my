...
class VisualizationDemo(object):
    ...
    def run_on_video(self, frames, score_thr=None,output_file=None):
        """
        Args:
        ...
        if score_thr is not None:
            pred_scores = torch.tensor(predictions["pred_scores"])
            pred_labels = torch.tensor(predictions["pred_labels"])
            pred_masks = torch.stack(predictions["pred_masks"], dim=0)

            valid_inds = pred_scores > score_thr
            ...
            pred_masks = [m for m in pred_masks]
        ...
        for frame_idx in range(len(frames)):
            output_json = os.path.join(output_file, f"{frame_idx}.json")
            ...
            import numpy as np
            import json
            masks = ins.pred_masks.cpu().numpy()
            labels = ins.pred_classes
            scores = ins.scores
            result = []
