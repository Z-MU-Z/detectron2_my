...
class OpenVocabVisualizer(Visualizer):
    def draw_panoptic_seg(self, panoptic_seg, segments_info, area_threshold=None, alpha=0.7):
        ...
        # write stuff_classes to a txt
        # with open('stuff_classes.txt', 'w') as f:
        #     for item in stuff_classes:
        #         f.write("%s\n" % item)
        ...
        # save instance masks to a json file
        result = []
        import pycocotools.mask as mask_util
        for mask, category_name in zip(masks, labels):
        ...
        import json
        with open(f"{output_dir}/{img_file_name.split('.jpg')[0]}.json", "w") as f:
            json.dump(result, f)
        try:
            colors = [
...
class VisualizationDemo(object):
    def __init__(self, cfg, instance_mode=ColorMode.IMAGE, parallel=False):
        ...
        stuff_classes = coco_stuff_classes + ade20k_stuff_classes # 53  + 50
        stuff_colors = coco_stuff_colors + ade20k_stuff_colors
        # NOTE: Mofified here
        # thing_classes = [''] + user_classes + coco_thing_classes + ade20k_thing_classes + lvis_classes #80 + 54 + 1203
        # thing_colors = [[0,0,0]] + user_colors + coco_thing_colors + ade20k_thing_colors + lvis_colors
        
        thing_classes =  lvis_classes #80 + 54 + 1203
    ...
    def run_on_image(self, image):
        ...
        OpenVocabVisualizer.img_file_name = self.img_file_name
        OpenVocabVisualizer.output_dir = self.output_dir
        if "panoptic_seg" in predictions:
            ...
            if "sem_seg" in predictions:
                # NOTE: Mofified here
                pmax, p = predictions["sem_seg"].max(dim=0)
                p[(pmax < 0.1) | (p > 300)] = 0
                vis_output = visualizer.draw_sem_seg(
                    p.to(self.cpu_device)
                )
